...
```mermaid
flowchart TD
  A[Initialize defaults in code] --> B[Parse command-line pass 1]
  B --> C[Load and apply config file values]
  C --> D[Parse command-line pass 2]
...
- command-line override: `-t` / `--timeout`.
- Normalization rules:
...
Result: command-line wins, then safety normalization may still adjust invalid values.
...
- command-line override: `-S` / `--stuffing`.
- Validation:
...
- Command-line options are intentionally parsed twice so command-line keeps highest precedence after config file load.
...
2. Use command-line only for temporary overrides.
